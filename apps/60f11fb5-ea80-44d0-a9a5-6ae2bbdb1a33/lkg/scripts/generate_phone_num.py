...
def run(context):
    # gather info
    us_area_code_cities = context.get_file_path("~/test_ai/apps/60f11fb5-ea80-44d0-a9a5-6ae2bbdb1a33/lkg/scripts/us-area-code-cities.csv`")
    # find a valid area code
    with open(us_area_code_cities) as f:
    	reader = csv.reader(f)
    	data = [row[col] for row in reader]
    
    # generate valid phone number and take action
    if element_exists(context, 'inp_phone_number'):
